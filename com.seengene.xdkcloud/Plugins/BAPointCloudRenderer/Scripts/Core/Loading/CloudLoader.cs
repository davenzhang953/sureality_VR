...
namespace BAPointCloudRenderer.Loading {
    ...
    class CloudLoader {
        ...
        public static PointCloudMetaData LoadMetaData(string cloudPath, string fullPath, bool moveToOrigin = false) {
            ...
            //Debug.Log(cloudPath);
            using (StreamReader reader = new StreamReader(fullPath + "cloud.js", Encoding.Default))
            {
            ...
            metaData.cloudFullPath = fullPath;
        ...
        private static byte[] FindAndLoadFile(string dataRPath, PointCloudMetaData metaData, string id, string fileending) {
            ...
            if (File.Exists(metaData.cloudFullPath + dataRPath + path)){
                return File.ReadAllBytes(metaData.cloudFullPath + dataRPath + path);
            }else if(metaData.cloudUrl != null){
                Directory.CreateDirectory(Path.GetDirectoryName(metaData.cloudFullPath + dataRPath + path));
                WebClient webClient = new WebClient();
                webClient.DownloadFile(metaData.cloudUrl + dataRPath + path, metaData.cloudFullPath + dataRPath + path);
                return File.ReadAllBytes(metaData.cloudFullPath + dataRPath + path);
            }
            return null;
        }
