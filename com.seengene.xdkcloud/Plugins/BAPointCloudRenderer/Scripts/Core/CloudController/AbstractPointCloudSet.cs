...
namespace BAPointCloudRenderer.CloudController
{
    ...
    /// </summary>
    [RequireComponent(typeof(DefaultMeshConfiguration))]
    public abstract class AbstractPointCloudSet : MonoBehaviour
    {
        ...
        public void resetAll()
        {
            ...
            waiterForBoundingBoxUpdate.Reset();
            moving = new Vector3d(0, 0, 0);
            overallBoundingBox = new BoundingBox(double.PositiveInfinity, double.PositiveInfinity, double.PositiveInfinity,
                                                                    double.NegativeInfinity, double.NegativeInfinity, double.NegativeInfinity);
            overallTightBoundingBox = new BoundingBox(double.PositiveInfinity, double.PositiveInfinity, double.PositiveInfinity,
                                                                    double.NegativeInfinity, double.NegativeInfinity, double.NegativeInfinity);
            boundingBoxes = new Dictionary<PointCloudLoader, BoundingBox>();
            tightBoundingBoxes = new Dictionary<PointCloudLoader, BoundingBox>();
        }
        ...
        {
            //Debug.Log("UpdateBoundingBox 11");
            initializedEvent.WaitOne();
            //Debug.Log("UpdateBoundingBox 12");
            ...
            //Debug.Log("UpdateBoundingBox 13");
            if (moveCenterToTransformPosition)
            {
                //Debug.Log("UpdateBoundingBox 14");
                waiterForBoundingBoxUpdate.WaitOne();
            }
            //Debug.Log("UpdateBoundingBox 15");
        }
        ...
        protected bool CheckReady()
        {
            ...
            lock (boundingBoxes)
            {
                if (!hasMoved)
                {
                    int counttt = boundingBoxes.Count;
                    Debug.Log("cui AbstractPointCloudSet CheckReady hasMoved? 11 count=" + counttt);
                    if (boundingBoxes.Count == 0)
                    {
                        //nothing to move along...
                        hasMoved = true;
                        Debug.Log("cui AbstractPointCloudSet CheckReady hasMoved? 12");
                    ...
                    else if (!boundingBoxes.ContainsValue(null))
                    {
                        ...
                        hasMoved = true;
                        Debug.Log("cui AbstractPointCloudSet CheckReady hasMoved? 13");
                        waiterForBoundingBoxUpdate.Set();
                    }
                    else
                    {
            ...
            lock (pRenderer)
            {
                ...
                int rootCount = pRenderer.GetRootNodeCount();
                if (rootCount != boundingBoxes.Count)
                {
                    return false;
                }
            }
