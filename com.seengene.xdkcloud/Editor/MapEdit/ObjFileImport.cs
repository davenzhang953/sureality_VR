...
public class ObjFileImport 
{
    ...
    public IEnumerator ImportFile(string filePath, Action<Mesh> callback, Action<float> onProgress)
    {
        yield return null;
        ...
        LoadMeshData(filePath, onProgress);
        ...
        {
            tempIndex = faceData[i].x - 1;
            if (tempIndex >= 0 && tempIndex < vertices.Count)
            {
                newVerts[i] = vertices[tempIndex];
            }

            tempIndex = faceData[i].y - 1;
            if (tempIndex >= 0 && tempIndex < uv.Count)
            {
                newUVs[i] = uv[tempIndex];
            }

            tempIndex = faceData[i].z - 1;
            if (tempIndex >= 0 && tempIndex < normals.Count)
            {
                newNormals[i] = normals[tempIndex];
            }
        }
    ...
    private void LoadMeshData(string fileName, Action<float> onProgress)
    {
        ...
        float step = text.Length / 200f;
        float send = 0f;
        for (int i = 0; i < text.Length; i++)
        {
            if (i > send + step)
            {
                float progress = i * 1.0f/ text.Length;
